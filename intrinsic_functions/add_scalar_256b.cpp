...
void vectorized(float* a, const float b, float* c, const uint64_t N)
{
    const uint64_t nb_iter = N / 8;

    for(auto i = 0; i < nb_iter; i++, a+=8, c+=8)
    {   
        _mm256_store_ps(c, _mm256_add_ps(*(__m256*)a, _mm256_set_ps(b, b, b, b, b, b, b, b)));
    }
}
